 ...
 *
 * THE single logo component for the entire app.
 * Uses the approved calligraphic W image asset (base64-embedded,
 * so no external file dependency — renders identically on all OS).
 *
 * Props:
 *   variant   "full"    — W + "Webale!" + "Private Group Fundraising"
 *             "compact" — W + "Webale!" only (navbar)
 *             "icon"    — W only (mobile / favicon contexts)
 *   theme     "light"   — navy W, dark text  (light backgrounds)
 *             "dark"    — white W, white text (dark/navy backgrounds)
 *   size      "xs"|"sm"|"md"|"lg"|"xl"  or a number (px height of W image)
...
export default function WebaleLogo({
  ...
  const nameColor    = theme === "dark" ? "#00E5CC" : "#00B4A0";
  const taglineColor = theme === "dark" ? "#FFB800" : "#E6A817";

  // Scale text proportionally with icon height
  const namePx    = Math.round(imgH * 0.32);
  const taglinePx = Math.round(imgH * 0.17);
